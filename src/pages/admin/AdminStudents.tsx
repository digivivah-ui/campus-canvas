...
export default function AdminStudents() {
  ...
  // Fees linked to a student (for profile)
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      if (!form.full_name || !form.course_id || !form.year_id || !form.semester_id) {
      ...
      if (editStudent) {
        const { error } = await supabase.from('students').update({
          ...
          paid_fees: Number(form.paid_fees) || 0,
  ...
  const toggleStatus = useMutation({
    mutationFn: async ({ id, status }: { id: string; status: string }) => {
      ...
      const { error } = await supabase.from('students').update({ admission_status: newStatus }).eq('id', id);
      if (error) throw error;
    },
  ...
  const openEdit = (s: Student) => {
    ...
    setForm({
      ...
      admission_status: s.admission_status || 'active',
    });
    setFormOpen(true);
  };
        ...
        {view === 'list' && (
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <Select value={filterCourse} onValueChange={v => { setFilterCourse(v); setFilterYear('all'); setFilterSemester('all'); setPage(1); }}>
              ...
              {filterCourse !== 'all' && (
                <Select value={filterYear} onValueChange={v => { setFilterYear(v); setFilterSemester('all'); setPage(1); }}>
                  ...
                  <SelectContent>
                    ...
                    {filterYears.map(y => <SelectItem key={y.id} value={y.id}>{y.name}</SelectItem>)}
                  </SelectContent>
                </Select>
              )}
              {filterYear !== 'all' && (
                <Select value={filterSemester} onValueChange={v => { setFilterSemester(v); setPage(1); }}>
                  ...
                  <SelectContent>
                    ...
                    {filterSemesters.map(s => <SelectItem key={s.id} value={s.id}>{s.name}</SelectItem>)}
                  </SelectContent>
                </Select>
              )}
                  ...
                  <TableBody>
                    {paginated.map(s => (
                      <TableRow key={s.id}>
                        ...
                        <TableCell>{s.semester_id ? getSemesterName(s.semester_id) : s.semester}</TableCell>
                        <TableCell>{s.phone || '-'}</TableCell>
                        <TableCell>
                          <Badge
                            variant={s.admission_status === 'active' ? 'default' : 'secondary'}
                            className="cursor-pointer"
                            onClick={() => toggleStatus.mutate({ id: s.id, status: s.admission_status })}
                          >
                            {s.admission_status}
                          </Badge>
                        </TableCell>
...
function StudentForm({
  form, setForm, courses, formYears, formSemesters, onSubmit, isPending, isEdit,
}: {
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <Label>Course *</Label>
            <Select value={form.course_id} onValueChange={v => setForm(p => ({ ...p, course_id: v, year_id: '', semester_id: '' }))}>
      ...
      <Button className="w-full" onClick={onSubmit} disabled={!form.full_name || !form.course_id || !form.year_id || !form.semester_id || isPending}>
...
function StudentProfile({
  student, getCourseName, getYearName, getSemesterName, fees,
}: {
  student: Student;
